...
//   USER CONFIG
// ----------------------------
#define BARO_RX_PIN 8      // AltSoftSerial RX is fixed on D8
#define BARO_TX_PIN 9      // AltSoftSerial TX is fixed on D9
...
void handleIncomingSerial() {
  while (baroSerial.available()) {
    ...
    if (rxLen + 1 < sizeof(rxLine)) rxLine[rxLen++] = c;
    else rxLen = 0;
  }
}
...
void drawTelemetryScreen() {
  ...
  if (ageMs < 2000) snprintf(buf, sizeof(buf), "LINK OK  %lums", ageMs);
  else snprintf(buf, sizeof(buf), "STALE    %lums", ageMs);
  writeRow(4, "");
  writeRow(5, buf);
  writeRow(6, "");
  writeRow(7, "");
...
void setup() {
  ...
  delay(200);
  Serial.println("Pro Mini OLED receiver");
  ...
  pinMode(BARO_RX_PIN, INPUT_PULLUP);
...
void loop() {
  ...
  if (telemetry.valid) drawTelemetryScreen();
