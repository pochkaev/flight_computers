...
// Telemetry V7: binary flight + nav packets
//
// Wiring summary:
...
// High-rate flight packet (5 Hz), dynamics only
struct __attribute__((packed)) FlightPacketV7 {
  uint8_t  version;    // 7
  uint8_t  state;      // FlightState
  uint16_t flags;      // bitfield

  uint32_t seq;        // flight packet counter
  uint32_t ms;         // ms since boot

  int32_t  alt_cm;     // baro altitude [cm]
  int16_t  vel_cms;    // vertical speed [cm/s]

  int16_t  ax_cms2;    // accel [cm/s^2]
  int16_t  ay_cms2;
  int16_t  az_cms2;

  int16_t  gx_cdeg;    // gyro [deg/s * 100]
  int16_t  gy_cdeg;
  int16_t  gz_cdeg;
...
// Low-rate nav packet (1 Hz), GPS & alt summary
struct __attribute__((packed)) NavPacketV7 {
  uint8_t  version;       // 7
  uint8_t  gps_fix_type;  // 0,2,3
  uint8_t  gps_sats;
  uint8_t  gps_hdop_x10;  // hdop * 10, clamped 0–255

  uint32_t seq;           // nav packet counter
  uint32_t ms;            // ms since boot

  int32_t  gps_lat_e7;    // lat * 1e7
  int32_t  gps_lon_e7;    // lon * 1e7
  int32_t  gps_alt_cm;    // GPS altitude [cm]
  int32_t  baro_alt_cm;   // current filtAlt [cm]

  uint32_t last_fix_age_ms; // ms since last good fix, 0xFFFFFFFF if never
};
static_assert(sizeof(NavPacketV7) == 32, "NavPacketV7 must be 32 bytes");

uint32_t packetSeq    = 0;  // flight seq
uint32_t navPacketSeq = 0;  // nav seq
...
void sendFlightTelemetry() {
  ...
  FlightPacketV7 pkt = {};  // zero everything
  ...
  pkt.vel_cms = (int16_t)lroundf(vel_ms * 100.0f);
  ...
  float roll_deg  = roll  * (180.0f / 3.14159265358979f);
  float pitch_deg = pitch * (180.0f / 3.14159265358979f);
  pkt.roll_cdeg  = (int16_t)lroundf(roll_deg  * 100.0f);
  pkt.pitch_cdeg = (int16_t)lroundf(pitch_deg * 100.0f);
  ...
  // Debug
  Serial.print("ROCKET TX FLIGHT SEQ=");
...
// ----------------- Telemetry send (nav/GPS) ---
void sendNavTelemetry() {
  if (loraTxBusy) {
    return;  // flight packets have priority
  }
  ...
  float hdop     = gpsHdop;
  int   hdop_x10 = (int)lroundf(hdop * 10.0f);
  if (hdop_x10 < 0)   hdop_x10 = 0;
  if (hdop_x10 > 255) hdop_x10 = 255;
  pkt.gps_hdop_x10 = (uint8_t)hdop_x10;

  pkt.seq = navPacketSeq++;
  pkt.ms  = millis();

  double lat = gpsLatDeg;
  double lon = gpsLonDeg;
  pkt.gps_lat_e7 = (int32_t)llround(lat * 1e7);
  pkt.gps_lon_e7 = (int32_t)llround(lon * 1e7);

  pkt.gps_alt_cm  = (int32_t)lroundf(gpsAltM * 100.0f);
  float baro_m    = filtAlt;
  pkt.baro_alt_cm = (int32_t)lroundf(baro_m * 100.0f);

  uint32_t nowMs = millis();
  if (haveGoodFix) {
    pkt.last_fix_age_ms = nowMs - lastFixTimeMs;
  } else {
    pkt.last_fix_age_ms = 0xFFFFFFFFu;
  }
  ...
  Serial.print("ROCKET TX NAV SEQ=");
  Serial.print(pkt.seq);
...
void setup() {
  ...
  Serial.println("Rocket V7 initialized");
}

void loop() {
  static uint32_t lastImuMicros = micros();
  static uint32_t lastStateMs   = millis();
  static uint32_t lastFlightMs  = millis();
  static uint32_t lastNavMs     = millis();
  ...
  // Flight telemetry at ~5 Hz
  if (nowMs - lastFlightMs >= 200) {
    lastFlightMs = nowMs;
    sendFlightTelemetry();
  }

  // Nav / GPS telemetry at ~1 Hz
  if (nowMs - lastNavMs >= 1000) {
    lastNavMs = nowMs;
    sendNavTelemetry();
  }
